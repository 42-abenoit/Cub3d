...
/*   Updated: 2020/09/12 11:58:20 by abenoit          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			apply_fog(double dist, int color_input, t_param *prm)
{
	...
	if ((color_input & 0x00FFFFFF) != 0)
	{
		...
		if (fog_percent > 1.0)
			fog_percent = 1.0;
		color = 0;
		color += apply_ratio_to_channel(color_input, fog_percent, conf->fog_color, 0x000000FF) & 0x000000FF;
		color += apply_ratio_to_channel(color_input, fog_percent, conf->fog_color, 0x0000FF00) & 0x0000FF00;
		color += apply_ratio_to_channel(color_input, fog_percent, conf->fog_color, 0x00FF0000) & 0x00FF0000;
	}
	return (color);
}
